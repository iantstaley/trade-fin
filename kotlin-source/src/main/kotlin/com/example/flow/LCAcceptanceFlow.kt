...
object LCAcceptanceFlow {
    ...
    class Initiator(val lcno: String, val lcState: String) : FlowLogic<SignedTransaction>() {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputSA = inState.withNewStatus(lcState)

            // Stage 4. Create the transfer command.
           // val signers = (inState.participants).map { it.owningKey }
            val signers = inState.issuingbank.owningKey
            val acceptCommand = Command(LCContract.Commands.Accept(), signers)
            ...
            builder.withItems(inputStateAndRef,
                    StateAndContract(outputSA, LCContract.LC_CONTRACT_ID),
                    acceptCommand)
            val counterParty =  inState.issuingbank
    ...
    @InitiatedBy(LCAcceptanceFlow.Initiator::class)
