...
class ExampleApi(private val rpcOps: CordaRPCOps) {
    ...
    /*    @GetMapping(value = "/remittances", produces = arrayOf("application/json"))
    fun getAllremittances():  List<Any> {

        val res = proxy.vaultQuery(IRState::class.java)
        logger.info(res.toString())
        val list = arrayListOf<Any>()
        for (state in res.states) {
            val d = state.state.data

            val remittance = getRemittanceDataFromState(d)
            list.add(remittance)
            logger.info(remittance.toString())
        }
        return list
    } */
}