...
fun main(args: Array<String>) {
    ...
    driver(DriverParameters(isDebug = true, waitForAllNodesToFinish = true)) {
        val (LuLuExchange, FederalBank: NodeHandle) = listOf(
                startNode(providedName = CordaX500Name("LuLuExchange", "Dubai", "IN"), rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("FederalBank", "Kochi", "IN"), rpcUsers = listOf(user))).map { it.getOrThrow() }

        startWebserver(LuLuExchange)
        startWebserver(FederalBank)
